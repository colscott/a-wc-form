...
import { binderRegistry, validatorRegistry as validator } from 'a-wc-form-binder';
...
function inputValue(controlId, value) {
  const controlElement = getControl(controlId).shadowRoot.querySelector('input');
...
describe('form-binder binding tests', () => {
  ...
  it('Should not change value when invalid', async () => {
    ...
    inputValue('age', '20');
    getControl('student').shadowRoot.querySelector('input').click();
