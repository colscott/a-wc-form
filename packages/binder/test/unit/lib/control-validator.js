    ...
    errors
      ...
      .forEach((controlValidationResult) => {
        ...
        control.setCustomValidity(
          controlValidationResults.map((controlValidator) => errorText[controlValidator.name]).join(','),
        );
      });
  });
...
export function standupValidatorTest(...validatorsToTest) {
  after(() => {
    validatorsToTest.forEach((validatorToTest) => validatorRegistry.remove(validatorToTest));
  });
